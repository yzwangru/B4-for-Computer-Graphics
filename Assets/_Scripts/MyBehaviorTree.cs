...
public class MyBehaviorTree : MonoBehaviour
{
	...
	{
		if(guy.tag!="Passenger") return new LeafInvoke(()=>turnlight(guy));
	...
	RunStatus turnlight(GameObject guy)
	{   if (guy.tag == "Daniel" || guy.tag == "Richard" || guy.tag == "Tom") {
			if (lightToSwitch1.enabled == true) {
				lightToSwitch1.enabled = false;
				lightToSwitch2.enabled = false;
				lightToSwitch3.enabled = false;
			} else {
